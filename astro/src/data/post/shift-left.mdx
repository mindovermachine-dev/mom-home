...
excerpt: There is a widespread trend in digital online presence that we should all be aware of US techbros' evil intentions. But this worry is not just relevant to SoMe, it is also relevant to the software development stack.
...
As always, we will work by an MVP (Minimum Viable Product) approach, and we may start with a small subset of the tools and use publicly and non-commercially available resources such as `matrix.to`, `matrix.org`, `codeberg.org`, and `mastodon.social`.
...
1. Go to our space: [#mindovermachine\:matrix.org](https://matrix.to/#/#mindovermachine:matrix.org).
2. Create an account on a public homeserver such as `matrix.org`, or sign in with an existing account.
3. Use a web client such as [element.io](https://app.element.io) or install a client such as FluffyChat or Element X, then say hi.

The space link is an open invite; you do not need a MoM account to join.
:::
